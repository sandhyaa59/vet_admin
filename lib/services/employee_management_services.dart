...
class EmployeeManagementServices {
  static Future<dynamic> getEmployeeManagementList(
      PaginationRequest request) async {
    try {
      Uri uri = Uri.parse(
          "${EndPoints.EMPLOYEEMANAGEMENT_LIST}?page=${request.page}&?pageSize=${request.pageSize}");
      ...
      if (res != null) {
          ...
          employeeManagementResponseFromJson(res);
  ...
  static Future<dynamic> addEmployee(EmployeeAddRequest addRequest) async {
    try {
      ...
      var response = await http.post(uri,
          headers: headers, body: employeeAddRequestToJson(addRequest));
      var res = handleResponse(response);
      if (res != null) {
        return res;
      } else {
  ...
  static Future<dynamic> updateEmployee(
      EmployeeUpdateRequest employeeUpdateRequest) async {
    try {
      var token = await StorageUtil.getValue("token");
